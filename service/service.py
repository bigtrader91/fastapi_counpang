...
def 연관검색어(keyword):
    ...
    url_keyword = "https://search.naver.com/search.naver?where=nexearch&sm=top_hty&fbm=0&ie=utf8&query=" + keyword
    req = requests.get(url_keyword ,headers=headers)
    soup = BeautifulSoup(req.text, 'html.parser')
    try:
        result = soup.find("div", {"class":"related_srch"}).find_all("li", {"class":"item"})
